...
// ── Pipeline Review content → paragraphs ─────────────────────────────────────
// Each non-empty line → paragraph + blank line after, so the output is
// clean and ready for copy-paste into Outlook with clear visual separation.
// No bullets, no special symbols.

function parsePipelineContent(docx, text) {
  if (!text || !text.trim()) return [];
  ...
  for (const line of text.split("\n")) {
    ...
    // Strip any accidental leading bullet chars from AI output
    const cleaned = trimmed.replace(/^[-•*]\s+/, "");
    if (!cleaned) continue;

    // Each point → its own paragraph, no blank lines between them
    paras.push(normal(docx, cleaned));
  }
  return paras;
}
...
export async function generateWordDoc(form) {
  ...
  if (form.salesBacklogCurrent && !isNaN(cur)) {
    ...
    if (form.salesBacklogPrev && !isNaN(prev) && prev !== 0) {
      ...
      backlogLine = `${formatted} Euro (${sign}${pct}% vs PY)`;
    } else {
      backlogLine = `${formatted} Euro`;
    }
    children.push(normal(docx, backlogLine));
  ...
  if (filledPipelines.length > 0) {
    ...
    for (const review of filledPipelines) {
      ...
      // If country is "__cliente__", use the custom label instead
      const countryName = review.country === "__cliente__"
        ? (review.customLabel || "Cliente")
        : review.country;
      const header = countryName
        ? `${countryName} - Pipeline Review`
        : "Pipeline Review";
      children.push(bold(docx, header));

      const contentParas = parsePipelineContent(docx, review.content);
      children.push(...contentParas);
      // Extra blank line between pipeline review blocks
      children.push(blank(docx));
    }
  }
